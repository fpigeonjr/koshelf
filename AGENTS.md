...
- **Documents Sync**: `/mnt/us/documents/` ↔ `~/Code/koshelf/data/books/`
  ...
  - Reading statistics and progress data embedded within .sdr files (no separate database sync required)
...
### Sync Locations
1. **Documents Directory** - EPUBs, metadata (.sdr folders), and reading statistics (embedded in .sdr files)
...
5. **No Database Sync** - Statistics database is computed from .sdr files, not synced separately
...
2. **Computed Cache**: `statistics.sqlite3` database
   - Generated by KOShelf from .sdr file data during site generation
   - Located at `/app/koreader-settings/data/statistics.sqlite3` in container
   - **Not synced via Syncthing** - rebuilt from source data automatically

#### Statistics Data Flow
```
KOReader Device → .sdr/metadata.epub.lua → Syncthing → KOShelf books directory → 
Statistics extraction → statistics.sqlite3 → Statistics/Calendar pages
```

#### Key Points for Agents
- **No separate statistics sync required** - all data flows through the books directory
- **Statistics database is expendable** - can be deleted and will regenerate from .sdr files
- **Sync only books directory** - contains all necessary data for full site generation
- **Calendar and statistics pages auto-update** - triggered by any .sdr file changes
- **Database modifications don't sync back** - KOShelf reads but doesn't write to .sdr files

#### Debugging Statistics Issues
- Check `.sdr` directory contents for recent modifications
- Verify statistics database path: `/app/koreader-settings/data/statistics.sqlite3`
- Monitor logs for "statistics_parser" and statistics database detection
- Statistics pages regenerate on every site build automatically
